...
// Sample road sign data (add more as needed)
window.roadSigns = [
  {
    name: "Stop",
    type: "Regulatory",
    subcategory: "Mandatory",
    description: "Indicates vehicles must stop and yield to other traffic.",
    img: "images/signs/stop.png"
  },
  {
    name: "Speed Limit 50",
    type: "Regulatory",
    subcategory: "Speed Limit",
    description: "Maximum speed limit is 50 km/h.",
    img: "images/signs/speed-limit-50.png"
  },
  {
  ...
  },
  {
    ...
    description: "Indicates a hospital is nearby.",
    img: "images/signs/hospital.png"
  }
];

// Render the sign gallery on learn.html
window.renderSignGallery = function renderSignGallery() {
  const gallery = document.getElementById('signGallery');
  if (!gallery) return;

  // Get unique categories and subcategories
  const categories = Array.from(new Set(window.roadSigns.map(sign => sign.type)));
  let selectedCategory = window.selectedSignCategory || categories[0];
  const subcategories = Array.from(new Set(window.roadSigns.filter(sign => sign.type === selectedCategory).map(sign => sign.subcategory)));
  ...
  const filteredSigns = window.roadSigns.filter(sign => sign.type === selectedCategory && sign.subcategory === selectedSubcategory);
...
// Use modularized LLR questions for the quiz
const quizQuestions = window.llrQuestions || [];
